...
static CGSize gBgSize;
static CGFloat gCornerRadius;
static CGFloat gFSize;
...
{
    gBgSize = CGSizeMake(125, 125);
    gCornerRadius = 12;
    gFSize = 25;
    return [MBHUDView _hudWithBody:body type:type hidesAfter:delay show:show];
}
...
+(MBHUDView*)_hudWithBody:(NSString*)body type:(MBAlertViewHUDType)type hidesAfter:(float)delay show:(BOOL)show{
    MBHUDView *alert = [[MBHUDView alloc] init];
    alert.bodyText = body;
    alert.hudType = type;
    alert.hudHideDelay = delay;
    alert.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.9];
    alert.addsToWindow = YES; //jovi modified
...
+(MBHUDView*)showError:(NSString*)body {
    ...
    if (body.length < 7 ) {
        return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeExclamationMark hidesAfter:1.5 show:YES bgSize:CGSizeMake(125,125) cornerRadius:20 fontSize:16];

    }
    else {
        return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeExclamationMark hidesAfter:1.5 show:YES bgSize:CGSizeMake(250,125) cornerRadius:20 fontSize:16];
...
+(MBHUDView*)show:(NSString*)body {
    CGSize size;
    if (IOS8) {
        size = [body sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]}];
    }else{
        size = [body sizeWithFont:[UIFont systemFontOfSize:16]];
    }
    return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeDefault hidesAfter:1 show:YES bgSize:CGSizeMake(size.width + 25, 40) cornerRadius:20 fontSize:16];
//    return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeActivityIndicator hidesAfter:0 show:YES bgSize:CGSizeMake(125,80) cornerRadius:20 fontSize:16];
...
//    }
    ...
    return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeCheckmark hidesAfter:1 show:YES bgSize:CGSizeMake(125, 125) cornerRadius:20 fontSize:16];
}

+(MBHUDView*)showWait:(NSString*)body{
    return [MBHUDView cusHudWithBody:body type:MBAlertViewHUDTypeActivityIndicator hidesAfter:0 show:YES bgSize:CGSizeMake(125,125) cornerRadius:20 fontSize:16];
}
...
-(UIFont*)bodyFont
{
    if(_bodyFont)
        return _bodyFont;
    CGFloat size = 0;
    [self.bodyText sizeWithFont:[UIFont boldSystemFontOfSize:gFSize] minFontSize:10 actualFontSize:&size forWidth:self.contentRect.size.width / 1.3 lineBreakMode:NSLineBreakByTruncatingTail];
...
-(void)loadView
{
    ...
    _backgroundButton.layer.cornerRadius = gCornerRadius;
}
